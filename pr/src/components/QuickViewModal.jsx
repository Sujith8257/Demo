  ...
  {item && <motion.div className="fixed inset-0 z-[500] bg-[#071916]/75 backdrop-blur-sm flex items-center justify-center p-4" role="presentation"
    ...
    <motion.div role="dialog" aria-modal="true" aria-label={item.title||"Product quick view"}
      ...
      <div className="grid sm:grid-cols-[220px_1fr] gap-6 items-center pt-5 sm:pt-0">
       <div className="aspect-square bg-[#ebeeee] rounded-xl overflow-hidden flex items-center justify-center">
        {item.image ? <img src={item.image} alt={item.title||"Watch"} className="h-full w-full object-contain" /> : <span className="material-symbols-outlined text-7xl text-[#717978]">watch</span>}
       </div>
       <div className="flex flex-col gap-3">
        ...
        <button type="button" onClick={ui.addModalToCart} className="bg-[#123b3a] text-white font-semibold py-3 px-5 rounded-xl hover:bg-[#002524] transition-colors">Add to preview cart</button>
       </div>
      </div>
    </motion.div>
  </motion.div>}
