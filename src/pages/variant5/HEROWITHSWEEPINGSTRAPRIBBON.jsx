  ...
  return (
    ...
    >
      ...
      <div className="relative w-full h-[70vh] sm:h-[80vh] lg:h-[calc(100vh-92px)] min-h-[520px] max-h-[900px] overflow-hidden">
        {/* Animated Background Image - Full brightness and clarity */}
        ...
        {/* Directional gradient overlay: darkens ONLY the left text area, right side is 100% transparent */}
        <div className="absolute inset-0 bg-gradient-to-t sm:bg-gradient-to-r from-black/85 via-black/50 sm:via-black/40 to-transparent z-10">
          <div className="w-full h-full max-w-[1320px] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-end sm:justify-center pb-14 sm:pb-0">
            {/* Open content area without opaque solid box to keep the photo completely visible */}
            <div className="max-w-xl lg:max-w-2xl py-4 sm:py-6">
              <span className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-black/60 backdrop-blur-md text-[#ff9f1c] border border-[#ff9f1c]/50 text-xs sm:text-sm font-bold uppercase tracking-wider mb-2.5 sm:mb-3 shadow-lg">
                <span className="w-2 h-2 rounded-full bg-[#ff9f1c] animate-pulse" />
                {banner.tag}
              </span>

              <h1
                className="font-poppins text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-white tracking-tight leading-[1.08] mb-3"
                style={{
              ...
              <p
                className="font-instrument text-white text-sm sm:text-base md:text-lg leading-relaxed mb-6 max-w-lg font-medium"
                style={{
              ...
              <div>
                <a
                  href={banner.href}
                  className="inline-flex items-center justify-center h-11 sm:h-12 px-7 sm:px-8 rounded-md bg-[#ff9f1c] hover:bg-[#ffa726] text-[#131a2c] font-bold text-sm sm:text-base transition-all shadow-2xl hover:shadow-[#ff9f1c]/30 hover:scale-105 active:scale-95 cursor-pointer"
                >
                  {banner.cta}
                </a>
              </div>
