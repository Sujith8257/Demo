...
export default function DesignTaskbar({ active, onChange }) {
 return (
  <nav aria-label="Preview five watch catalogue designs"
    className="fixed bottom-3 sm:bottom-5 left-1/2 -translate-x-1/2 z-[300] w-[min(96vw,780px)] rounded-2xl bg-[#102e2b]/96 border border-[#b9c9c4]/30 px-2 sm:px-3 py-2 sm:py-2.5 shadow-[0_15px_55px_rgba(0,0,0,.32)] backdrop-blur-xl text-white">
   <div className="flex gap-1 sm:gap-2 items-center">
    <span className="hidden md:flex flex-col pl-2 pr-4 border-r border-white/20 shrink-0">
      <span className="text-[10px] uppercase tracking-[.16em] text-[#c9b17a] font-bold">AMIHIVE</span>
      <span className="text-xs font-semibold text-white/80">Design Preview</span>
    </span>
    {designs.map(d => (
